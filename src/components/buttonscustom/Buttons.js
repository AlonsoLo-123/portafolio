...
export const ButtonMenu = ({text, path}) => {
    ...
    return (
        <button className={`w-[97px] h-full mx-1 pt-2 bg-transparent focus:outline-none hover:pb-2 group active:pb-0 ${location.pathname === path ? 
            ...
            <div className={`border-t-2 mt-2 scale-75 ${location.pathname === path ? 
                'pb-2 border-white' : 
                'border-transparent group-hover:border-green-lime/75 animate-pulse'}`}/>
        </button>
    )
}
...
export const ButtonGit = ({imgSource, text, url}) => {
    return (
        <button onClick={() => {window.location.href = url}} className='flex flex-row w-[115px] h-[50px] bg-transparent hover:bg-white/50 border-2 focus:outline-0 border-lime-300 justify-center px-[5%]'>
            <img src={imgSource} className="flex w-full h-full object-contain"/> 
